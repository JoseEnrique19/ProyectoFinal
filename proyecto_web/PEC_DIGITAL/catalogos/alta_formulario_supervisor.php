...
?>
<script type="text/javascript" language="javascript" src="resources/js/paginas/catalogos/alta_formulario_supervisor.js"></script>
<br/><br/>
<form id="formFormularioFirmar">
...
INNER JOIN formularios f ON df.formulario = f.idFormulario INNER JOIN asignacionaf af ON af.formulario = f.idFormulario WHERE f.idFormulario =". $idFormulario;
                ...
                while ($rs = mysqli_fetch_array($query)) {
                    if (isset($_POST['id']) && $_POST['id'] != "") {
                        ...
                        echo "<tr>";
                            ...
                            if ($i == 2 || $i == 3) {
                                echo "<input type=\"".$rs['tipo']."\" style=\"width: 70%;\" class=\"redondea\" name=\"campo[".$i."]\" id=\"campo[".$i."]\" value=\"".$valor[$j]."\" placeholder=\"escribe\">";
                            }else{
                                echo "<input type=\"".$rs['tipo']."\" required=\"required\" style=\"width: 70%;\" class=\"redondea\" name=\"campo[".$i."]\" id=\"campo[".$i."]\" value=\"".$valor[$j]."\" placeholder=\"escribe\" readonly=\"readonly\">";
                            }
                            echo "</td>";
                        echo "</tr>";
                        break;                 
    ...
    <input type="button" id="cancelar" class="boton" name="cancelar" value="Cancelar" onclick="cambiarContenidosSinSesion('catalogos/lista_registros_supervisor.php', 'Formulario');"/>
</form>